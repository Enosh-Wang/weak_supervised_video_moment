...
class mapping(nn.Module):
    def __init__(self, opt, in_channels, out_channels , kernel_size, padding = 0, stride = 1):
    ...
    def forward(self, v_map, words, w_mask):
        
        # plot_map(v_map.cpu().detach().numpy(),'scdm')
        v_map = self.conv(v_map).relu()
        v_map = self.modulation(v_map,words,w_mask)
