...
class VideoCaption(nn.Module):
    def __init__(self, opt, vocab):
        ...
        self.opt = opt
        #self.fc = nn.Linear(opt.joint_dim, opt.word_dim) # 将视频帧映射到和词向量相同的维度 default=300

        self.rnn = nn.LSTM(opt.joint_dim, opt.joint_dim, num_layers=opt.RNN_layers)
    ...
    def forward(self, video_embedding, caption_word_embedding, caption_length):
        ...
        #video_embedding = self.fc(video_embedding) #[1,b,c]
        ...
        # PackedSequence
        #video_embedding = video_embedding.transpose(0,1)
...
class VideoCaption1(nn.Module):
    def __init__(self, opt, vocab):
        super(VideoCaption, self).__init__()
        self.idx2word = vocab.idx2word
        self.word2idx = vocab.word2idx
        self.opt = opt
        #self.fc = nn.Linear(opt.joint_dim, opt.joint_dim) # 将视频帧映射到和词向量相同的维度 default=300
    ...
    def forward(self, video_embedding, caption_word_embedding, caption_length):
        ...
        video_embedding = video_embedding.view(b,c,-1)
        video_embedding = torch.sum(video_embedding,dim=-1).unsqueeze(0) #[1,b,c]
        #video_embedding = self.fc(video_embedding).unsqueeze(0) #[1,b,c]
        # 将img_feats看成第一个词的词向量 
        caption_word_embedding = caption_word_embedding.transpose(0,1) #[l,b,c]
        video_embedding = video_embedding.expand_as(caption_word_embedding)
        #caption_word_embedding = torch.cat([video_embedding, caption_word_embedding], 0)
